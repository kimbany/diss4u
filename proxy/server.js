...
2. 입력된 [키워드]를 "뜻 그대로" 정확히 해석해서 전부 자연스럽게 녹여야 한다.
   ...
   ★ 예외 — "도저히 원형으로는 문맥이 안 살 때만" 의미를 살린 웃긴 표현으로 변형 허용:
     ...
     - 변형 시 반드시 두 조건을 지켜라:
       (가) 키워드의 핵심 의미가 그대로 살아 있어야 한다 (다른 뜻으로 바꾸지 마라).
       (나) 같은 곡 안에서 그 키워드가 한 번이라도 "원형"으로 등장하면 더 좋다. 원형 0회는 가급적 피해라.
     - 예: 키워드 "주말마다 잠수타고 집에서 안 나옴" (긴 서술형)
       → 원형은 노래에 안 맞음 → 좋은 변형: "주말엔 또 잠수네 또" / "주말마다 잠수, 집 밖 0%"
       (핵심 단어 "주말", "잠수" 보존됨)
...
긴 도입·서론·배경 설명 금지. **첫 줄부터 핵심 놀림이 시작**돼야 한다.
...
Hook은 이 노래의 90%다. Verse보다 압도적으로 강해야 한다.
이 곡의 흥행 여부는 전적으로 Hook의 중독성에 달렸다.

★ Hook은 "TikTok에서 들으면 따라 부르고 싶어지는 짧은 밈"이어야 한다.

Hook A 문장 조건 (4~7글자 권장, 길어도 10글자 이내):
* 들으면 바로 따라 외칠 수 있을 만큼 짧음
* 별명·동사·의성어 중심 (예: "또 늦잠 챔피언", "벽지가 운다")
* 리듬감 — 박자에 딱 맞게 끊김
* 같은 음절·라임이 두세 번 등장하면 더 끈적함 (예: "또 그러네 또 그러네")
* 한국어 의성어/추임새 활용 권장 (어이~, 또또또, 어머어머, 헐, 야야야)

Hook B 문장 조건 (8~14글자 권장):
* A를 받아치는 약오르는 한마디
* 과장 또는 반전 (예: A "냉장고 또 열어" → B "냉장고가 너 알아본다")
* 단톡방에서 따라 쓰고 싶은 멘트
...
★ 중독성 자체 검토 — Hook을 쓰고 나서 아래를 스스로 확인해라:
1. 이 두 줄을 다섯 번 연속으로 머릿속에서 부를 수 있나? → 못 부르면 다시 작성
2. 친구가 단톡방에 적었을 때 자연스럽게 ㅋㅋ 나오나? → 안 나오면 다시
3. 첫 음절이 강한가? (앞 글자가 약하면 임팩트 0) → 강한 음절로 시작하게 다시
4. 8글자를 넘어가면 무조건 줄이거나 잘라라

이 검토에서 한 개라도 실패하면 Hook을 다시 작성해야 한다.
...
Hook이 전체의 60% 이상을 차지해야 후렴이 귀에 박힌다.
Verse를 늘리고 싶은 욕구를 절대 따르지 마라. Verse는 무조건 짧게.
...
* 전체 13줄 (Verse 3 + Pre-Hook 2 + Hook 4 + Hook 반복 4)
* Verse 3줄
* Pre-Hook 2줄
...
아래 JSON 형식으로만 응답해라.
설명 문장은 절대 쓰지 마라.
lyrics는 총 13줄: Verse 3줄 + Pre-Hook 2줄 + Hook 8줄(A/B/A/B/A/B/A/B').
Hook 8줄을 반드시 채워라. Verse는 4줄로 늘리지 마라 (인트로가 길어지는 원인).

{
"title": "2~8글자 중심의 짧고 킹받는 노래 제목. 키워드 나열 금지. 놀리는 별명이나 한마디처럼 작성",
"style": "선택된 장르 특징 + 다음 키워드 반드시 포함(영어로): 'vocals start immediately at 0:00, no instrumental intro, no buildup, hook-driven', 'short 40-second TikTok meme song', 'addictive chant-able hook played twice', 'punchy rhythm, strong downbeat, catchy repetition', genre-specific rhythm/instruments/vocal style. ⚠️ 가장 중요: 인트로(악기 도입) 절대 금지 — 첫 1초 안에 보컬이 시작되어야 함",
"lyrics": "[Verse]\n...\n...\n...\n\n[Pre-Hook]\n...\n...\n\n[Hook]\nA\nB\nA\nB\nA\nB\nA\nB'"
}
`;
}
