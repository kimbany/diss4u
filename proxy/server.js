...
Intro는 기본적으로 넣지 마라. 첫 줄부터 바로 Verse로 시작하는 게 원칙이다.
정 넣고 싶어도 딱 1줄(2~3단어 추임새)까지만 — 절대 여러 줄로 길게 쓰지 마라.
...
★ 이 노래는 Hook 중심의 숏폼 디스곡이다. Verse·Pre-Hook은 빌드업용이고, Hook이 70% 이상을
   차지해야 중독성이 생긴다. **Pre-Hook은 가능하면 생략하라**(없는 게 기본).

권장 구조 (둘 중 하나, 60% 확률로 첫 번째를 선택해라):

A. **Hook 직진형 (권장 — 가장 중독성 높음)**
[Verse]    ← 2~3줄로 짧게
[Hook]     ← 8~10줄, 반복+변형 풍부

B. 풀 구조 (장면 빌드업이 꼭 필요한 경우에만)
[Verse]       ← 2~3줄
[Pre-Hook]    ← 1줄만 (몰아가기 한 방)
[Hook]        ← 8~10줄

Intro는 절대 넣지 마라. 첫 줄부터 Verse로 바로 시작해서 빠르게 Hook으로 진입한다.
(앞서 Intro 항목은 더 이상 사용 안 함.)

권장 줄 수:
* Verse: **2~3줄** (절대 4줄 넘기지 마라 — 짧을수록 좋음)
* Pre-Hook: **0~1줄** (대부분 생략, 꼭 필요할 때만 1줄)
* Hook: **8~10줄** (이 노래의 메인. 반복+변형이 풍부해야 함)

전체 가사는 10~14줄 이내. Hook이 전체의 70% 이상을 차지해야 한다.

---

[Intro 규칙]

★ Intro는 넣지 않는 것이 기본이다 (위 [노래 구조 규칙]과 동일). Hook의 중독성이 훨씬 중요하다.

정 넣고 싶을 때만 — 딱 1줄, 2~3단어짜리 짧은 추임새 1개까지만 허용한다.
여러 줄 Intro·설명형 Intro·상황 설명은 절대 금지(곡 앞부분이 길어져 지루해진다).

허용되는 1줄 추임새 예 (이 중 하나만, 짧게):
* 아르릉!
* 요를레이히!
* 야야!
...
★★★ 키워드 반복 규칙 (Hook의 영혼) — 이게 중독성의 핵심 ★★★

Hook의 메인 모티프(반복되는 짧은 구호 A)는 **입력 키워드를 살린 짧은 문장**이어야 한다.
입력 키워드가 노래 전체를 끌고 가는 "구호"가 되도록 만들어라.

규칙:
1. 키워드 중 가장 임팩트 있는 1개를 골라 그대로 또는 살짝 변형해 A 라인의 핵심으로 박아라.
2. A 라인은 **3~6음절 이내**로 짧게 — 입에 착 붙어야 한다 (말소리 흉내 가능한 짧은 리듬).
3. A 라인은 Hook 안에서 **최소 4번 이상 그대로 반복**돼야 한다 (변형 없이 똑같이).
4. 변주는 A 사이의 B 라인에서만 — 다양한 장면/상황으로 B를 바꿔라.
5. 노래 제목도 A 라인이거나 A 라인의 핵심 단어로 잡아라 (제목 = 후크 = 키워드 연결).
...
★ A 라인이 노래의 정체성이다. 듣는 사람이 "그 노래 그거지? '또 자네' 그거" 하고
  떠올릴 수 있어야 한다. 그러려면 A를 짧게, 같게, 자주 반복해라.
...
[클리셰 금지 — 같은 키워드여도 매번 다른 비유로]

LLM은 같은 키워드를 받으면 같은 비유로 가는 경향이 강하다. 이건 절대 안 된다.

★ 흔한 클리셰 (이런 소재·비유 전부 금지 — 누구나 1순위로 떠올리는 것들):

  · '못생겼다' / '못난이' / '얼굴' → **거울·셀카·사진·보정앱·정면샷** 전부 금지
    (셀카/보정앱도 이미 식상한 새 클리셰다. 외모를 "비추는 도구"로 풀지 마라.)
  · '키 작은' → 의자/까치발/높은 곳에 손 안 닿음 금지
  · '잠 많은' → 알람 시계/베개 금지
  · '많이 먹는' → 냉장고/통통한 배 금지
  · '게으른' → 침대/이불 금지
  · '욕쟁이' → 입에서 욕이 튀어나오는 묘사 금지(이미 욕설 금지에서 다룸)

  ※ 일부러 "대신 이걸 써라"라고 정해주지 않는다 — 정해주면 그게 곧 새 클리셰가 되기 때문.
    위 시드의 **'이번 곡 무대(장면)'** 안에서, 그 키워드가 그 상황에서 어떻게 드러나는지를
    구체적이고 의외인 장면으로 직접 만들어내라. (예: '못생김' + 무대 '소개팅' →
    상대가 메뉴판으로 얼굴 가리는 장면 / '못생김' + 무대 'CCTV' → 화질이 죄가 되는 장면)

★ 매번 다른 각도로 접근하는 7가지 도구상자 — 시드 6번을 따라 1~2개 골라 써라:

  1) 일상 장면 — 구체적인 시간/장소의 순간 (예: 화요일 점심 12시 30분, 회의실)
  2) 주변 반응 — 친구·가족·동료가 보는 시선 ("얘들아 또 그래")
  3) 본인 자각 — 스스로 알고 있으면서 변명하는 모습
  4) 비교/대비 — 다른 사람과 다른 점 ("쟨 하루에 한 끼, 너는 다섯 끼")
  5) 의성어/효과음 — 소리/움직임으로 묘사 (쿵, 헉, 꾸벅꾸벅)
  6) 시간 흐름 — 아침·낮·밤 변화로 같은 행동 반복
  7) 도구/물건 — 그 사람이 자주 쓰는 물건이 주인공 (배달 앱, 충전기, 후드티)

★ 자체 검토:
   - 같은 키워드로 다른 사람이 만든 평범한 노래에 나올 법한 비유인가? → 그러면 다시 써라.
   - 가사 안에 같은 비유를 두 번 쓰지는 않았는가?
   - "거울/알람/이불/냉장고" 같은 클리셰 단어가 안 들어갔는가?
