...
이전에 만든 곡과 다른 색을 내기 위해 이번 곡은 아래 4가지를 반드시 따라라.
...
★★★ K-POP 음절 반복 후크 규칙 (필살기) ★★★

위의 "A 라인 4회 반복" 위에 **한 단계 더 중독성 있는 음절 후크**를 만들어라.
키워드의 핵심 음절을 사운드처럼 늘리고 반복해서 "뜻"이 아니라 "소리"로 박힌다.
이게 K-pop 메가히트들이 쓰는 핵심 후크 패턴이다.

레퍼런스 (구조만 참고, 그대로 복사 금지):
   소녀시대 Gee       → "지지지지 베베베베베베베"
   샤이니 링딩동       → "링딩동 링딩동 링디기디기디딩딩딩"
   에스파 슈퍼노바     → "수수수수 수퍼노바"
   요들 캐릭터        → "야를르르르 야르 야를레이히"
   가상 키워드 "밤티"  → "밤밤바라밤밤바 티티티"

키워드 → 음절 후크 변형 패턴 4가지 (반드시 1가지 이상 적용):

1. **음절 반복형** "AAAA B"
   첫 음절을 4번 반복 후 단어 완성.
   - "수퍼노바" → "수수수수 수퍼노바"
   - "잠많은" → "잠잠잠잠 자네"
   - 키워드 ${name === '예시' ? '예' : '○○'} 첫 음절 → "○○○○ + 키워드"

2. **음절 분해 확장형** "A 라라 A 라라 B"
   음절 사이에 의성어/꾸밈음을 끼워 늘림.
   - "밤티" → "밤 바라 밤 바라 티티티"
   - "단추" → "단 디리 단 디리 추추추"

3. **의성어 확장형** "A 디기디기 A 딩딩딩"
   음절을 의성어로 분해 확장.
   - "링" → "링디기디기디딩딩딩"
   - "또" → "또띠띠띠 또또또"

4. **추임새 결합형** "A 르르르 A 추임새"
   음절을 늘이고 캐릭터 추임새 추가.
   - "야르" → "야를르르르 야르 야를레이히" (요들풍)
   - "또" → "또오오오 또자 또자야"

규칙:
- A 라인의 "뜻"이 100% 명확하지 않아도 된다. **사운드와 리듬이 우선**.
- 의미 없는 음절(라라, 디기디기, 딩딩딩, 르르르)을 적극 활용.
- 입력 키워드의 가장 임팩트 있는 음절을 골라 변형해라.
- 장르 캐릭터에 맞춰 변형해라:
   · 요들 → "야를레이히" 같은 요들 추임새
   · 트로트/뽕짝 → "둠칫둠칫", "짠짠"
   · 힙합/랩 → "오오우", "예에"
...
[Hook 권장 구조 — 음절 후크 적용 예시]
   (음절 후크) AAAA B / 또는 A 라라 A B
   B (장면)
   (음절 후크 반복)
   B (장면)
   (음절 후크 반복)
...
이 음절 후크가 Hook의 영혼이다. **사운드가 먼저, 뜻은 그 다음.**
