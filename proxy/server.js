...
키워드를 평범하게 풀어쓰지 말고,
아래 기법들을 활용해서 한 번 더 비틀어라.
(아래 예시 문장들은 기법 설명용이다. 그대로 복사하지 말고, 매 곡의 키워드에 맞게 새로 만들어라.)

1. 의인화 — 사물이 그 사람 때문에 고통받는 것처럼 표현
   * 늦잠 → "알람이 먼저 지쳐서 포기했대"
   * 먹보 → "냉장고가 너만 보면 떨고 있어"
   * 폰 중독 → "배터리가 살려달라고 빌고 있어"

2. 공식 기록/타이틀 수여 — 그 특징을 수상 경력처럼 말하기
   * 지각 → "3년 연속 지각 부문 대상 수상"
   * 먹보 → "배달앱이 인정한 VIP 등급"
   * 길치 → "네비도 포기한 전설의 길치"

3. 주변의 반응으로 표현 — 본인이 아니라 주변이 어떻게 됐는지로 놀리기
   * 코골이 → "옆집에서 공사하냐고 민원 들어와"
   * 발냄새 → "양말이 스스로 빨래통에 걸어 들어가"
   * 목소리 큼 → "옆 동네까지 다 들었대"

4. 인터넷 밈 화법 — 요즘 단톡방/SNS에서 놀릴 때 쓰는 말투 활용
   * "~하는 거 실화냐"
   * "이게 맞나 싶다"
   * "폼 미쳤다 (나쁜 의미로)"
   * "레전드 갱신했다"
   * "~그 자체"
   * "현실 (특징) 장인"
   * 밈 화법은 곡당 1~2번만. 과하면 유치해진다.
   * 너무 옛날 유행어("헐", "대박", "킹왕짱")는 쓰지 마라. 오히려 안 웃기다.

5. 진지한 톤으로 어이없는 내용 말하기 — 뉴스/다큐처럼 심각하게
   * 먹보 → "오늘도 한 그릇이 사라졌습니다"
   * 늦잠 → "본 기자는 그가 일어나길 3시간째 기다리고 있다"

6. 칭찬인 척 디스 — 앞은 칭찬, 뒤에서 뒤집기
   * "꾸준한 건 인정해, 맨날 늦는 게"
   * "한결같아서 좋아, 살 안 빼는 게"
   * "재능 있어, 핑계 대는 거"

7. 숫자로 과장 — 구체적인 숫자가 들어가면 더 웃기다
   * "알람 17개 다 무시함"
   * "배달앱 주문 횟수 신기록"
   * "5분 거리를 40분 걸려 옴"

이 기법들 중 매 곡마다 2~3개를 골라 섞어 써라.
같은 기법만 반복하지 마라.
