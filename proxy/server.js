...
function buildPrompt(params) {
  ...
  const sceneSeedLine =
    ...
    + `   (예: '맵찔이'=매운 거 못 먹음 → 분식집·마라탕집·라면 끓일 때·집 O / 버스 안에서 먹기 X)\n`
...
❌ 나쁜 예 — 키워드(맵찔이·길치·허당)를 Hook에 다 욱여넣어 B라인 주제가 튐:
   또 맵찔이 / 물 어딨어 외치네 / 또 맵찔이 / 지도 켜도 또 헤매네(←길치) / 또 맵찔이 / 허당 폼 미쳤네(←허당)
   → B라인이 맵찔이→길치→허당으로 튀어 안 이어지고 반복감이 없다. 길치·허당은 Verse로 빼라.

⭕ 좋은 예 — 메인 '맵찔이' 하나로만 변주 (길치·허당은 Verse에서 다룸):
   또 맵찔이          ← A
   물부터 찾네         ← B (맵찔이)
   또 맵찔이          ← A
   우유를 들이켜네      ← B (맵찔이)
...
❌ 밋밋한 B라인 (그냥 묘사 — 재미·중독성 없음):
   또 맵찔이 / 물부터 찾네
   또 맵찔이 / 땀 뻘뻘 또 졌네
⭕ 킹받는 B라인 (과장·반전·말장난 — 단, 누구나 바로 이해되는 것):
   또 맵찔이 / 김치도 씻어 먹네
   또 맵찔이 / 우유가 주식이네
   또 맵찔이 / 엽떡은 눈으로만
   또 맵찔이 / 물 세 잔 추가요
★ 핵심1: 듣고 "푸하 뭐야 ㅋㅋ" 소리가 나와야 한다. 정보 전달(X) → 웃음 포인트(O).
★ 핵심2: 비유는 "바로" 이해돼야 한다. 무슨 뜻인지 한 번 더 생각해야 하는 비유는 실패다.
   (나쁜 예: "신라면도 외국어" ← 무슨 말인지 안 와닿음 / 좋은 예: "김치도 씻어 먹네" ← 즉시 이해+웃김)
   그리고 B라인끼리 끝소리(라임)를 맞추면 더 잘 붙는다.
