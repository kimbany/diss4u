...
[노래 구조 규칙]
가사는 약 40~50초 분량. 인트로는 쓰지 말고, 처음부터 바로 대상의 특징·행동이 드러나는 Verse로 시작해라.
구성: Verse(6~8줄) / Pre-Hook(2~4줄) / Hook(4~6줄) / Outro(1~2줄).
- [Verse] 대상의 특징과 상황을 장면으로 바로 보여준다. 키워드를 단순 나열 말고 실제 어떤 사람인지 보여줘라.
- [Pre-Hook] 놀림 포인트를 더 세게 몰아 "아 얘 진짜 이렇다니까?" 느낌을 만든 뒤 Hook으로 넘긴다.
- [Hook] 가장 킹받는 별명이나 문장을 중독성 있게 반복한다.
- [Outro] 마지막에 한 방 먹이는 짧은 마무리. 갑자기 끊기지 않고 짧게 끝맺는 느낌.
2절·긴 브릿지·긴 엔딩 금지. 흐름: 상황 제시 → 놀림 강화 → 중독성 Hook → 짧은 마무리. 틱톡/릴스서 반복재생될 중독성.

--------------------------------------------------

[Hook 작성 규칙]
Hook/Chorus에서도 키워드를 단순 반복하지 마라. 대신 놀리는 별명 / 반복되는 행동 / 친구들이 맨날 하는 놀림 말투 / 들으면 킹받을 한마디로 변형해라.
"키워드 반복"보다 "킹받는 별명화"가 더 중요하다.
나쁜 Hook: "야근러 다이어터 힘순찐이 / 야근러 다이어터 힘순찐이"(키워드 그대로 반복).
좋은 Hook: "말만 관리 중이야 / 말만 관리 중이야 / 김밥천국 VIP야 / 내일부터 다이어터야"(별명·말투로 변형).
...
- Verse 1 / Pre-Hook 1 / Hook 1 / Outro 1. Intro 금지. 같은 섹션 반복 금지. 전체 13~20줄 이내.

--------------------------------------------------

[제목 생성 규칙]
제목은 친구를 놀리는 앱에 어울리게 짧고 웃기고 킹받게 작성해라.
키워드를 나열하지 말고, 대상의 특징을 한 번에 떠올릴 수 있는 "놀리는 별명" 또는 "킹받는 한마디"처럼 작성해라.
...
규칙:
- 2~8글자를 우선하고, 길어도 12글자를 넘기지 마라.
- 모든 키워드를 억지로 넣지 말고, 가장 웃긴 키워드 1~2개만 골라 제목화해라.
- 아래 스타일 중 하나로: ①별명형(특징을 별명처럼) ②한마디형(약올리듯 던지는 말) ③밈형(반복해 부르고 싶은 짧은 말) ④반전형(말은 멀쩡한데 은근 킹받음).
- 강한 욕설/혐오/인격 비하 금지. 귀엽게 약오르고 "아 뭐야 ㅋㅋ" 할 정도로.

좋은 예: "또 먹네" / "김밥천국 VIP" / "단추 비상" / "옆구리 테러범" / "하얀 솜뭉치" / "내일부터 다이어트" / "꼬집고 튀어" / "지각 장인" / "허당 모닝콜" / "말만 관리 중".
나쁜 예(키워드 나열·길고 노잼): "덤벙이 야근러 다이어터 힘순찐" / "올챙이배 꼬집기 대마왕" / "하얀개 귀여운 강아지 밥먹기".
(예시 단어는 작성법 설명용일 뿐, [키워드]에 없으면 제목에도 쓰지 마라.)
