...
function buildPrompt(params) {
  ...
  // 이름 사용 규칙: 끄면(false) 제목에만, 켜면(true) 가사 본문에 실제로 호명하도록 강제.
  const nameRule = (useNameInLyrics === false)
    ? `\n\n[이름 사용 규칙]\n이름(${name})은 제목(title)에만 사용할 수 있고, 가사(lyrics) 본문에는 절대 쓰지 마라.\n가사에서는 "너", "쟤", "걔" 같은 지시어로만 가리켜라.\n`
    : `\n\n[이름 사용 규칙]\n★ 이름(${name})을 가사(lyrics) 본문에 최소 2번 이상 직접 불러라(호명). 특히 Hook에서 한 번은 "${name}"이 나와야 한다.\n지시어(너/쟤/걔)만으로 때우지 말고, 이름을 불러서 누구를 놀리는지 분명히 특정해라.\n단, 모든 줄에 욱여넣지는 말고 임팩트 있는 자리(Hook 시작/끝 등)에 자연스럽게 배치해라.\n`;

  // ── 매번 다른 곡 — 창작 시드 ─────────────────────────────────────
  // 같은 입력이어도 호출마다 Hook 패턴/감정 톤/추임새/문장 스타일을 랜덤으로 골라 프롬프트에
  // 끼워넣는다. Claude/Solar가 시드를 따라 색다르게 작곡하도록 강제.
  const HOOK_PATTERNS = [
    'A B A B A B A B (가장 단순한 반복 — 머리에 박힘)',
    'A A B A (A 세 번, 끝에 B로 변주 — 대중적 후렴)',
    'A B C A B C (3줄 묶음을 두 번 — 풍성)',
    'A B A B + 마지막에 변형 라인 1줄 (반복 → 한 방 마무리)',
  ...
  const MOODS = [
    ...
    '시크하고 무덤덤한 톤 (한숨 섞인 디스)',
    '신난 장난 톤 (놀리면서 즐거워 죽는 분위기)',
    '체념한 듯 웃긴 톤 ("또 시작이네" 류)',
    '의문문 위주의 묻듯이 놀리는 톤 (왜? 진짜? 또?)'
  ];
  const ADLIB_STYLES = [
    '추임새 자주 — Hook 반복마다 다른 추임새, 신나는 분위기',
    '추임새 적게 — Hook 클라이맥스에서만 1~2번',
    '추임새 없이 가사로만 승부 (말맛 위주)',
    '의성어 추임새 (둠칫둠칫, 짠짠, 빰빠라밤)',
    '감탄사 추임새 (헐, 와, 진짜로, 어머)',
    '캐릭터 효과음 추임새 (멍멍, 야옹, 삑삑, 뿌뿌)'
  ];
  const SENTENCE_STYLES = [
    '평서문 위주 — 짧고 단호하게 끝맺기',
    '의문문 위주 — 묻듯이 놀리기 (왜? 또? 진짜?)',
    '명령형/금지형 섞기 (그만, 해라, 마라, 안 돼)',
    '감탄형 섞기 (좀! 진짜! 와!)',
    '말 끊어가기 — 짧은 끊김감 (~네, ~잖아, ~다고)'
  ];
  // 음절 후크 모드 — 매 곡 K-POP 음절 분해 후크를 쓸지 말지/얼마나 쓸지 결정.
  //   · 'no_syllable' 비중을 크게(약 50%) — 일반 한국어 후크가 기본이 되게.
  //   · 'syllable_strong'은 가끔만 — 너무 자주 나오면 비슷한 후크가 됨.
  const SYLLABLE_HOOK_MODES = [
    { tag: 'no_syllable',     desc: '음절 분해 후크 사용 안 함 — 일반 한국어 후크로 자연스럽게 작성' },
    { tag: 'no_syllable',     desc: '음절 분해 후크 사용 안 함 — 일반 한국어 후크로 자연스럽게 작성' },
    { tag: 'no_syllable',     desc: '음절 분해 후크 사용 안 함 — 일반 한국어 후크로 자연스럽게 작성' },
    { tag: 'syllable_light',  desc: 'K-POP 음절 분해 후크를 1줄만 가볍게 끼워넣기 (나머지는 일반 후크)' },
    { tag: 'syllable_light',  desc: 'K-POP 음절 분해 후크를 1줄만 가볍게 끼워넣기 (나머지는 일반 후크)' },
    { tag: 'syllable_strong', desc: 'K-POP 음절 분해 후크를 메인 모티프로 강하게 사용 (Gee/링딩동 스타일)' }
  ];
  // 음절 후크 패턴 풀 — strong/light일 때 매번 다른 패턴이 선택되도록 7종으로 확장.
  // 4종만 있으면 같은 패턴 반복 → 다양화 위해 새 3종 추가.
  const SYLLABLE_PATTERNS = [
    'AAAA B — 첫 음절 4번 반복 후 단어 완성 (예: 수수수수 수퍼노바)',
    'A 라라 A 라라 B — 음절 사이 의성어 꾸밈음 (예: 밤 바라 밤 바라 티티티)',
    'A 디기디기 A 딩딩딩 — 음절 의성어 분해 확장 (예: 링디기디기디딩딩딩)',
    'A 르르르 A 추임새 — 음절 늘이기 + 캐릭터 추임새 (예: 야를르르르 야르 야를레이히)',
    'A-A-A 점프 B — 같은 음절 짧게 3번 + 단어 (예: 잠-잠-잠 자네)',
    'A 오우 A 예 B — R&B 스타일 보컬 애드립 사이에 끼움 (예: 단추 오우 단추 예 비상)',
    'AB-AB-AB 콤보 — 두 음절을 한 호흡으로 짧게 반복 (예: 밤티-밤티-밤티)'
  ];
  // 가사 접근 각도 — LLM 클리셰 회피용. 같은 키워드여도 매번 다른 각도로.
  const APPROACH_ANGLES = [
    '일상 장면 위주 — 구체적인 순간/시간/장소를 묘사',
    '주변 반응 위주 — 친구·가족·동료가 보는 시선',
    '본인 자각 위주 — 스스로 인지하고 변명하는 모습',
    '비교/대비 위주 — 다른 사람과 다른 점',
    '의성어/효과음 위주 — 소리/움직임으로 묘사',
    '시간 흐름 위주 — 아침/낮/밤 변화로 같은 행동 반복',
    '특정 상황 위주 — 회식/회의/약속/카톡 같은 명확한 신(scene)',
    '도구/물건 위주 — 그 사람이 자주 쓰는 물건이 주인공',
    '소셜 미디어 위주 — 카톡·인스타·셀카 같은 디지털 흔적'
  ];
  // 이번 곡의 "무대(장면)" — 같은 키워드라도 매번 다른 상황/장소에서 풀게 강제.
  // 클리셰(거울·셀카) 탈출의 핵심 장치: 곡마다 완전히 다른 세계에 떨어뜨린다.
  const SCENE_DOMAINS = [
    '소개팅·맞선 자리', '졸업앨범/여권사진 찍는 날', '회사 면접장', '단톡방·DM 대화창',
    '지하철·버스 안', '편의점·무인 키오스크 앞', '헬스장/운동하는 중', '명절 가족 모임',
    '배달앱·택배 받는 순간', '카페 구석자리', '노래방', '동창회·모임 2차',
    '엘리베이터 안의 정적', 'CCTV·블랙박스에 찍힌 장면', '병원 대기실', '시험장/자격증 공부방',
  ...
  // 곡 전개 방식(포맷) — 매 곡 다른 "노래 형식"으로 만들어 '다 비슷한 느낌'을 깬다.
  // 디스(놀림)는 유지하되 구조·톤이 곡마다 달라진다. (선택 장르 사운드는 어떤 포맷에서도 유지)
  const SONG_FORMATS = [
    '훅 직진형 — 짧은 Verse + 반복 후렴. 중독성 최강(클래식). 구조: Verse 2~3줄 + Hook 6~10줄',
    '스토리텔링형 — 시간순/사건 전개로 놀림(아침→점심→밤 등). Verse 비중↑, 후렴은 짧게',
    '과장 드라마형 — 비장하고 진지한 척 과장해 놀림. 발라드/락에 잘 어울림. 후렴은 한 방으로',
    '펀치라인 나열형 — 짧고 센 디스 라인을 빠르게 쏟아냄. 힙합/랩에 잘 어울림. 끝에 후렴 한 줄',
    '콜앤리스폰스형 — 묻고 답하기/주고받기로 놀림 (다들 ○○? / 너만 ○○!)',
    '응원가·칭찬 역설형 — 칭찬·응원하는 척하며 놀림("잘한다 잘해"). 떼창 후렴',
    '챈트 구호형 — 짧은 떼창 구호 중심. 응원 구호처럼 반복. 중독성↑',
    '반전 빌드업형 — 잔잔/진지하게 시작 → 후반에 한 방 터뜨림',
    '인터뷰·자막형 — 본인 인터뷰/내레이션 같은 말투로 놀림 (TV 자막 느낌)'
  ];
  ...
  // 장면은 구체적으로 그리되, 키워드 행동에 "자연스러운" 장소만 쓰게 유도(억지 장소 방지).
  const sceneSeedLine =
      `7. 장면 구체화: 키워드를 추상적으로 말하지 말고 "구체적인 순간·장소"에서 보여줘라.\n`
    + `   ★ 단, 그 키워드 행동에 자연스러운 장소·상황만 골라라. 억지스러운 장소는 금지.\n`
    + `   (예: '늦잠'=아침에 못 일어남 → 침대·알람 앞·출근길·주말 O / 수영장에서 늦잠 X)\n`
    + `   (예: '길치' → 골목·지하철 환승·주차장에서 헤맴 O / 헬스장에서 길 잃기 X)`;
  const seedBlock = `

═══════════════════════════════
★ 이번 곡의 색깔 (Creative Seed — 매 호출마다 다르게 주어진다) ★
═══════════════════════════════

이전에 만든 곡과 다른 색을 내기 위해 이번 곡은 아래를 반드시 따라라.
같은 키워드라도 시드가 다르면 결과 노래가 달라져야 한다.

★★★ 이번 곡 전개 방식(포맷): ${seed.format} ★★★
   → 이 포맷대로 곡을 만들어라. 매 곡 포맷이 달라서 "다 비슷한 느낌"이 사라져야 한다.
   → ★ 단, 선택한 [장르]의 사운드·분위기는 이 포맷 안에서도 그대로 살려라.
   → ★ 그리고 어떤 포맷이든 "한 번 들으면 따라하고 싶은 한 줄(후렴/구호)"은 꼭 하나 남겨라(중독성).
   → 아래 1~7번 시드는 이 포맷을 "어떻게 색칠할지" 정하는 보조 양념이다.

1. Hook(후렴) 반복 패턴: ${seed.hook}
...
═══════════════════════════════
위 7개 시드는 "양념"이다. 그러나 가장 중요한 건 가사 전체가 **하나의 장면·이야기로 매끄럽게 이어지는 것**이다.
- 한 줄 한 줄이 앞 줄에서 자연스럽게 이어져야 한다. 따로 노는 문장을 나열하지 마라.
- 시드(무대·각도·감정 등)를 다 욱여넣다가 말이 안 이어지면, **시드를 덜 넣더라도 흐름을 살려라.**
- 시드는 곡의 "색깔"을 다르게 할 뿐, 문맥을 깨는 핑계가 아니다.
특히 5번 음절 후크 모드는 매번 달라진다 — 'no_syllable'이면 음절 분해 후크를 절대 쓰지 마라.
`;
...
2. 입력된 [키워드]를 "뜻 그대로" 정확히 해석해서 전부 자연스럽게 녹여야 한다.
   ...
   ★★ 키워드 원형 유지 — 완화 버전 (★ 문맥과 Hook의 중독성이 항상 우선):
     - 모든 키워드의 "의미"는 반드시 가사에 반영해라.
     - 짧고 자연스러운 단어형 키워드(늦잠/지각/발냄새/집순이/욕쟁이/ISTJ/계획충/읽씹 등)는
       가능하면 원형 그대로 최소 1회 등장시키는 것을 우선한다.
       좋은 예: "또 늦잠이야 너", "발냄새 한 방에 KO", "ADHD 풀가동 모드"
     - 단, 키워드가 너무 길거나 문장형이거나, 원형 그대로 넣으면 가사 흐름이 깨지는 경우엔
       핵심 단어만 보존하거나 의미를 살린 짧은 표현으로 바꿔도 된다.
     - 중요: 원형을 억지로 박는 게 아니라 "그 키워드가 이렇게 웃기게 살아났네"라고
       느끼게 만드는 것이 목표다. 원형 유지보다 문맥과 Hook의 중독성이 우선이다.
...
노래 전체는 반드시 1분 이하로 끝나야 한다. 가장 이상적인 길이는 40~55초이다.
2절, 긴 Bridge, 긴 Outro, 긴 엔딩은 금지한다.
(Intro는 아래 [Intro 규칙] 참조 — 기본적으로 넣지 않는다.)

★★ 가사 = 노래 일치 규칙 (매우 중요) ★★
가사가 너무 많으면 Suno가 45초 안에 다 못 불러서 줄을 건너뛰거나 뭉갠다
→ "써 있는 가사 ≠ 부르는 가사"가 된다. 이걸 막으려면:
* 가사는 ★45초 안에 전부 또박또박 불릴 분량★만 써라. 짧을수록 일치율이 높다.
* 가사 줄 수를 욕심내지 마라. 한 줄도 안 잘리고 다 불리는 게 더 중요하다.
* 후렴(반복 구호)도 너무 여러 번 쓰지 마라 — 반복이 많으면 뒷부분이 잘린다.
...
이 노래는 1분 이하 숏폼 디스곡이다. ★ 구조는 위 시드의 '전개 방식(포맷)'을 따른다.
  · 훅 직진형 / 챈트 구호형 / 응원가형 → 짧은 Verse + 반복 후렴 (Hook 6~10줄, 중독성 최우선)
  · 스토리텔링형 / 과장 드라마형 / 반전 빌드업형 → Verse(전개) 비중을 키우고 후렴은 짧게
  · 펀치라인 나열형 → 짧고 센 라인을 빠르게 쌓고 끝에 후렴 한 줄
  · 콜앤리스폰스형 / 인터뷰·자막형 → 주고받기·내레이션 흐름에 맞게 자유롭게

공통 원칙 (포맷 무관, 반드시 지킴):
* 전체 8~12줄 이내(짧을수록 좋음). ★45초 안에 다 불릴 분량을 절대 넘기지 마라★ — 넘으면 가사가 잘려 노래와 안 맞는다.
* ★ 어떤 포맷이든 "한 번 들으면 따라하고 싶은 한 줄(후렴/구호)"을 반드시 하나 만들어라 (중독성).
* ★ 선택한 [장르]의 사운드·창법·분위기는 어떤 포맷에서도 그대로 살려라.
* Pre-Hook은 선택. 넣어도 1줄만.

---

[Intro 규칙]

Intro는 기본적으로 사용하지 않는다. 첫 줄부터 Verse로 바로 시작하는 것이 원칙이다.
단, 장르 특성상 꼭 필요할 때만 괄호 추임새 1줄을 Verse 앞에 넣을 수 있다.
예: (요를레이히!) (얼쑤!) (야야!) (둠칫!)

이 경우에도 [Intro] 섹션은 만들지 마라 — 추임새 한 줄로만 처리한다.
여러 줄 Intro·설명형 Intro·긴 상황 설명은 절대 금지. 길어도 10초를 넘기면 안 된다.

---

[Verse 규칙]

Verse는 입력 키워드(특히 보조 키워드)를 짧은 장면으로 녹이는 구간이다.
설명문이 아니라, Hook으로 가기 전의 짧은 "증거 장면"이어야 한다. 길어지면 안 된다.
...
나쁜 Verse:
* 이 친구는 지각을 자주 하고 알람을 잘 듣지 않으며 커피를 들고 뛰어오는 척을 한다

한 줄에 하나의 포인트만. 키워드를 욱여넣어 말이 어색해지면 실패다.

---

[Pre-Hook 규칙]

Pre-Hook은 Hook 직전의 몰아가기 1줄이다. 기본적으로 생략해도 된다. 넣어도 1줄만.
짧고 리듬감 있게. 예: 시계도 포기했네 / 오늘도 딱 걸렸네 / 배부터 등장했네

---

[Hook 규칙]

(아래는 "후렴/따라부르는 한 줄·구호"를 만들 때 규칙이다 — 포맷에 후렴이 있으면 이대로 만들어라.
 스토리텔링·드라마·나열형처럼 후렴이 짧은 포맷이어도, 그 짧은 후렴만큼은 이 규칙을 따른다.)

Hook은 입력 키워드 중 가장 강한 1개(메인 키워드)만 골라 만든다.
메인 키워드 = 가장 웃기고 / 짧게 반복하기 좋고 / 따라 부르기 쉽고 / 제목으로 써도 되는 것.

★★ Hook은 "메인 키워드 1개의 세계"로만 채운다 (가장 중요) ★★
- A라인은 물론 B라인도 전부 그 메인 키워드 하나의 주제 안에서만 변주해라.
- ★ 보조 키워드(나머지)는 Hook에 끌어오지 마라 — 보조 키워드는 Verse에서 다룬다.
- Hook이 한 가지 주제로 반복돼야 "그 노래 그거!"가 되고 중독성이 생긴다.
  B라인마다 다른 키워드로 갈아끼우면 내용이 안 이어지고 반복감이 죽는다.

★ A라인 = 메인 키워드를 살린 짧은 구호 (3~8글자 / 4~9음절). 너무 길면 실패.
★ A라인은 Hook 안에서 최소 3회 이상 반복(이상적 4회). 변형 없이 똑같이.
★ B라인 = A 사이에서 "같은 메인 키워드"를 다른 장면·반전·말장난으로 변주 (주제는 안 바뀜).
★ 제목도 A라인(또는 그 핵심 단어)으로 잡아라 (제목 = 후크 = 키워드 연결).

권장 Hook 구조:  A B A B A B A B'
마지막 B'는 곡이 끝나는 느낌으로, 그 곡의 키워드·상황에 맞게 새로 작성해라(예시 복사 금지).

(아래 예시는 키워드 '늦잠'으로 구조만 보여준다 — ★ 문장 그대로 복사 금지 ★)
❌ 나쁜 예 — 키워드(늦잠·길치·계획충)를 Hook에 다 욱여넣어 B라인 주제가 튐:
   또 늦잠 / 알람 또 껐네 / 또 늦잠 / 지도 켜도 헤매네(←길치) / 또 늦잠 / 계획표만 빼곡(←계획충)
   → B라인이 늦잠→길치→계획충으로 튀어 안 이어지고 반복감이 없다. 길치·계획충은 Verse로 빼라.

⭕ 좋은 예 — 메인 '늦잠' 하나로만 변주 (길치·계획충은 Verse에서 다룸):
   또 늦잠           ← A
   베개랑 한 몸이네     ← B (늦잠)
   또 늦잠           ← A
   정오에 눈 뜨네      ← B (늦잠)
...
[K-POP 음절 분해 후크 규칙 — 시드의 '5. 음절 후크 모드'에 따라 적용]

★ 이 섹션은 무조건 적용하는 게 아니다. 이번 곡의 시드(위 Creative Seed 5번)를 따른다.

  · 시드가 'no_syllable' → **이 섹션 전체를 무시**하고 일반 한국어 후크로만 만들어라.
    A 라인은 자연스러운 한국어 표현(예: "또 자네", "단추 비상")이어야 한다.
  · 시드가 'syllable_light' → 시드에 지정된 음절 패턴을 후크에서 1~2회 가볍게 사용.
    나머지 후크는 일반 한국어 후크.
  · 시드가 'syllable_strong' → 시드에 지정된 음절 패턴을 메인 모티프로 강하게 사용
    (K-POP 메가히트 Gee/링딩동/슈퍼노바 스타일).

레퍼런스 패턴 (구조만 참고, 그대로 복사 금지):
   소녀시대 Gee       → "지지지지 베베베베베베베"        (AAAA B 패턴)
   샤이니 링딩동       → "링딩동 링딩동 링디기디기디딩딩딩"   (A 디기디기 패턴)
   에스파 슈퍼노바     → "수수수수 수퍼노바"               (AAAA B 패턴)
   요들 캐릭터        → "야를르르르 야르 야를레이히"        (A 르르르 + 추임새)

음절 후크 작성 시 규칙:
- 시드에 지정된 패턴 **1가지만** 사용해라 — 여러 패턴 섞지 마라.
- 키워드의 가장 임팩트 있는 음절을 골라 변형.
- 의미 없는 음절(라라, 디기디기, 딩딩딩, 르르르) 또는 장르 캐릭터 추임새 활용:
   · 요들 → "야를레이히", 트로트/뽕짝 → "둠칫둠칫·짠짠"
   · 힙합/랩 → "오오우·예에", K-pop → "라라라·오오오"
   · 발라드 → "오오오~·음~"

★ 음절 후크는 **시드가 허용할 때만** 쓰는 양념이다. 매번 강제로 쓰면 노래들이 다 비슷해진다.

Hook은 반드시:
* 짧은 문장 (특히 A는 3~6음절)
* 강한 반복 (A 라인 4회 이상 동일하게)
* 말맛 있는 표현 (B 라인은 말꼬리 살짝 다르게)
* 따라 부르기 쉬운 리듬
* 친구가 들으면 킹받는 한마디
* 제목으로 써도 될 만큼 선명한 표현
으로 작성해라.
...
흔한 키워드를 받았을 때, 가장 먼저 떠오르는 뻔한 비유로 도망가지 마라.
  · 못생김 → 거울·셀카·보정앱만 쓰기 금지
  · 잠 많음 → 알람·베개만 쓰기 금지
  · 게으름 → 침대·이불만 쓰기 금지
  · 많이 먹음 → 냉장고만 쓰기 금지
  · 키 작음 → 까치발·높은 선반만 쓰기 금지

★ 예외 (중요): 사용자가 입력한 키워드 자체에 그 단어가 들어 있으면 사용해도 된다.
   예) 키워드가 "알람 무시"면 알람 OK / "이불 밖은 위험해"면 이불 OK / "셀카 중독"이면 셀카 OK.
   단, 이 경우에도 흔한 표현을 그대로 쓰지 말고 새로운 장면·말맛·언어유희로 비틀어라.

목표는 "뻔한 단어 자체를 금지"하는 게 아니라 "뻔한 표현을 그대로 쓰지 않게" 만드는 것이다.
위 시드의 '이번 곡 무대(장면)'를 활용하면 자연스럽게 신선한 장면이 나온다.
(예: '못생김' + 무대 '소개팅' → 상대가 메뉴판으로 얼굴 가리는 장면)

★ 매번 다른 각도로 접근하는 7가지 도구상자 — 시드 '접근 각도'를 따라 1~2개 골라 써라:
  1) 일상 장면   2) 주변 반응   3) 본인 자각   4) 비교/대비
  5) 의성어/효과음   6) 시간 흐름   7) 도구/물건

★ 자체 검토: 다른 사람 노래에도 나올 법한 뻔한 비유면 다시 써라.
   (단, 키워드 자체에 포함된 단어는 위 예외에 따라 사용 가능.)
...
★★ B라인 "한 방" 규칙 (재미의 핵심) ★★
Hook의 B라인은 사실을 그냥 묘사하면 밋밋하고 안 웃기다.
B라인마다 아래 중 하나로 "한 방"을 넣어라 → 과장 / 반전 / 말장난 / 의외의 비유 / 받아치기.

(아래 예시는 키워드 '늦잠'으로 "기법"만 보여주는 것 — ★ 이 문장·소재를 그대로 베끼지 마라 ★)
❌ 밋밋한 B라인 (그냥 묘사 — 재미 없음):
   또 늦잠 / 알람을 껐네
   또 늦잠 / 또 지각이네
⭕ 킹받는 B라인 (과장·반전·말장난, 누구나 바로 이해되는 것):
   또 늦잠 / 알람이 자장가네
   또 늦잠 / 점심에 첫 끼네
   또 늦잠 / 이불이 접착제네
   또 늦잠 / 해가 중천 클리어
★ 핵심1: 듣고 "푸하 뭐야 ㅋㅋ" 소리가 나와야 한다. 정보 전달(X) → 웃음 포인트(O).
★ 핵심2: 비유는 "바로" 이해돼야 한다. 한 번 더 생각해야 하는 비유는 실패(예: "신라면도 외국어" ← 안 와닿음).
★★ 핵심3 (가장 중요 — 매번 비슷해지는 것 방지): 위 예시 문장·소재를 가사에 그대로 쓰지 마라.
   예시는 발상 "방식"만 보여준다. 반드시 네가 새 문장을 만들어라.
   - 같은 키워드를 또 받아도 매번 "다른 디테일"을 새로 발굴해라.
   - 가장 뻔한 1순위 디테일만 반복하지 마라 (예: '맵찔이'=물·우유·땀·김치, '늦잠'=알람·이불).
     2~3순위의 의외 디테일·상황을 파라. 매번 같은 소재가 나오면 실패다.
   - B라인끼리 끝소리(라임)를 맞추면 더 잘 붙는다.
...
사용자가 [키워드]에 유행어나 인터넷 밈 단어를 입력했을 때,
그 단어의 정확한 뜻과 뉘앙스를 이해하고 가사에 반영해라.

유행어를 단순히 단어로 박지 말고,
그 단어가 의미하는 상황·감정·행동을 가사로 풀어내라.

[유행어 이해 예시]

* 킹받다 → 약 오르고 짜증나지만 웃기다 / 어이없어서 한 번 더 보게 됨
   → 가사에서 활용: "또 킹받게 늦었네", "킹받음 풀가동 모드"
* 폼 미쳤다 → 잘하거나 한심한 정도가 어이없을 때 쓰는 반어법
   → "지각 폼 미쳤다", "야식 폼 살아있네"
* 갓생 → 매일을 알차게 사는 모범적인 삶 (반대어로 자주 쓰임)
   → "갓생 산다더니 또 늦잠"
* 현타 → 현실 자각 타임 / 갑자기 정신이 드는 순간
   → "장바구니 보고 현타 왔네"
* 찐친 → 진짜 친구, 가까운 친구
* MZ → 요즘 세대 / 새로운 가치관 가진 세대
* 어쩔티비 / 저쩔티비 → 무시하는 말장난, 가볍게 받아치기
* 오저치고 → 오늘 저녁 치킨 고? (제안형 줄임말)
* 알잘딱깔센 → "알아서 잘 딱 깔끔하고 센스있게"
* JMT → 존맛탱 (맛있다는 강조)
* 인정? 어 인정 → 동의/공감 강조
* 별다줄 → 별걸 다 줄임
* 1도 없다 → 전혀 없다
* 핵인싸/아싸 → 매우 인기 많은 사람 / 사회 활동 안 하는 사람
* 갑분싸 → 갑자기 분위기 싸해짐
* 깐부 → 친한 친구, 동맹
* 찐텐 → 진심 텐션
* 인생 ○○ → 인생 최고의 ○○ (인생 음식, 인생 영화 등)

[유행어 사용 규칙]

1. 의미 정확히 이해 — 단어만 보지 말고, 그 단어가 왜 웃긴지를 가사에 녹여라.
   - 나쁜 예: "킹받다"를 단순히 단어로 박기 → "오늘도 킹받다"
   - 좋은 예: 킹받음의 상황을 그려내기 → "야 또 그러네 킹받게"

2. 곡당 1~2개만 — 유행어를 너무 많이 쓰면 유치해진다.
   언어유희·말맛의 양념으로만 사용해라.

3. 너무 오래된/식상한 유행어는 피해라.
   금지 예시: 헐 / 대박 / 킹왕짱 / 완전 짱 / 안녕하세요 호머 / 즐(즐ㅋ)
   (이런 표현은 이미 죽은 유행어라 촌스러움)

4. 키워드에 입력된 유행어는 반드시 사용 — 입력 키워드의 원형 유지 규칙(위 ★★)이
   우선 적용된다. 사용자가 "킹받다"를 키워드에 넣었으면 가사에 "킹받"이 등장해야 한다.

5. 유행어와 키워드는 충돌하지 않게 — 유행어는 표현 방식이고,
   키워드는 놀림의 소재다. 키워드(예: "늦잠")를 유행어(예: "폼 미쳤다")로 표현 가능:
   → "지각 폼 미쳤다" / "늦잠 갓생 클래스"

6. 새로 만들어내지 마라 — 실제 사용되는 유행어만 써라.
   AI가 그럴듯하게 지어낸 가짜 유행어는 어색하다.
...
2단계 — 그 키워드가 일상에서 드러나는 "장면" 떠올리기
   예: 단추가 안 잠겨서 입씨름하는 장면, 소개팅에서 메뉴판으로 얼굴 가리는 장면, 배 만지면서 다이어트 결심
...
10. 요들송 (yodel)
    ★★ 요들의 핵심 = "키워드를 요들식 꾸밈 음절로 늘린 후크" ★★
       이게 요들 곡의 정체성이다. 요들이면 (시드의 음절 모드와 무관하게) 반드시 이 후크를 넣어라.

    [키워드 → 요들 꾸밈 음절 만드는 법]
    · 요들 필러: 르 / 를 / 루 / 리 / 레 / 이  +  렁 / 레이히 / 레이리 / 룰루 / 르르르
    · 예) 키워드 "야르" →  야르렁 · 야를르 · 야르루리 · 야르레이히 · 야를레리룰루
    · 예) 키워드 "맵찔이" →  맵찔이야르 · 찔이릴리 · 찔이레이히 · 맵찔루리
    · 리듬 반복: "야르 야르 야르 야르" / "야르루리 야르루리 야르루리"
    · 요들링 굴림(올라갔다 꺾기): "야를르르르 야르 야를레이히"

    [반드시 지킬 것]
    · ★ 전체를 옹알이로 만들지 마라 — 누굴 놀리는지 알 수 있게, "뜻 있는 놀림 줄"과
      "요들 꾸밈 음절"을 사이사이 교대로 배치해라 (뜻 줄 : 꾸밈 = 대략 반반).
    · ★ 꾸밈 음절은 반드시 "그 키워드 발음"에서 파생돼야 한다 (아무 라라라 X).
    · 짧은 키워드는 통째로, 긴 키워드는 핵심 음절만 뽑아 굴려라.
    · 밝고 익살스러운 알프스풍, "요들레이 / 요를레이히" 추임새도 양념으로.
...
출력하기 전에 스스로 아래를 확인하고, 하나라도 부족하면 다시 써라.
1. Hook의 메인 키워드가 1개로 선명한가?
2. A라인이 짧고 반복하기 쉬운가?
3. A라인이 최소 3회 이상 반복되었는가?
4. Verse에서 나머지 키워드가 자연스럽게 살아났는가?
5. 모든 줄이 앞뒤로 자연스럽게 이어지는가? (문맥 — 최우선)
6. 한 줄에 정보가 너무 많이 들어가지 않았는가?
7. 설명문처럼 들리는 줄은 없는가?
8. 말맛이나 언어유희가 있는가?
9. Intro가 없거나 아주 짧은가?
10. 전체가 1분 이하 숏폼 노래처럼 느껴지는가?
11. 약오르지만 상처 주는 표현은 아닌가?
...
아래 형식 중 하나를 사용해라 (★ A형을 우선 — Hook 중심).
...
★ Intro는 절대 넣지 마라. Verse 2~3줄 → 바로 Hook 8~10줄.
