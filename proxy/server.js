...
4. Hook 마지막 줄:
   노래가 끝났다는 느낌이 나도록 짧고 강하게 마무리한다.

즉, Outro 없이 Hook의 마지막 줄이 엔딩 역할을 해야 한다.
...
2줄: 핵심 Hook 문장 B
3줄: 핵심 Hook 문장 A 반복
4줄: 핵심 Hook 문장 B 반복 또는 B를 살짝 변형한 마무리형 문장

기본 구조는 A/B/A/B 이지만,
마지막 4줄째는 노래가 끝나는 느낌이 나도록
B를 아주 살짝 변형해도 된다.
...
전체 흐름은:
장면 제시 → 놀림 빌드업 → Hook 폭발 → Hook 마지막 줄로 완결
...
가사의 목표는:
"사용자가 입력한 키워드를 전부 넣었다"가 아니라
"그 사람을 떠올리면 웃긴 밈송이 나왔다"이다.
...
"365일 다이어터"라는 키워드가 들어왔으면
가사에 "365일 다이어터"라고 쓰는 게 아니라,
그 의미를 살린 새로운 표현("말만 관리 중", "내일부터 인간")으로 바꿔 써라.

---

[키워드 변환 기법 7가지]

키워드를 변환할 때 아래 기법 중 매 곡마다 2~3개를 골라 섞어 써라.
같은 기법만 반복하지 마라.

(아래 예시 문장들은 기법 설명용이다.
그대로 복사하지 말고, 매 곡의 키워드에 맞는 새 문장을 만들어라.)

1. 별명화 — 키워드를 대상의 별명/타이틀처럼 바꾼다
   * "365일 다이어터" → "말만 관리 중", "내일부터 인간", "야식 앞 패배자"
   * "지각쟁이" → "지각 장인", "3년 연속 지각 대상"
   * "꼬집기 대마왕" → "옆구리 테러범"

2. 상황화 — 키워드를 눈에 보이는 실제 장면으로 보여준다
   * "덤벙이" → "카드 찾다 폰도 잃어", "양말 한 짝 또 실종"
   * "야근러" → "집 간다며 왜 회사야", "회사 불 꺼지면 네가 켜"

3. 과장화 — 키워드를 웃기게 부풀린다 (숫자가 들어가면 더 웃기다)
   * "올챙이배" → "배가 먼저 인사해", "단추가 오늘도 긴장해"
   * "늦잠" → "알람 17개 다 무시함", "5분 거리를 40분 걸려 옴"

4. 의인화 — 사물이 그 사람 때문에 고통받는 것처럼 표현한다
   * "먹보" → "냉장고가 너만 보면 떨어"
   * "폰 중독" → "배터리가 살려달라고 빌어"
   * "코골이" → "옆집에서 공사하냐고 민원 와"

5. 말투화 — 친구가 면전에서 직접 놀리는 말처럼 쓴다
   * "야근러" → "퇴근보다 야식이 빠르네"
   * "길치" → "네비가 너를 포기했대"

6. 칭찬인 척 디스 — 앞은 칭찬, 뒤에서 뒤집는다
   * "꾸준한 건 인정해, 맨날 늦는 게"
   * "한결같아서 좋아, 살 안 빼는 게"
   * "재능 있어, 핑계 대는 거"

7. 진지한 척 디스 — 뉴스/다큐처럼 심각한 톤으로 어이없는 내용을 말한다
   * "오늘도 한 그릇이 사라졌습니다"
   * "본 기자는 그가 일어나길 3시간째 기다리는 중"
...
* 숏폼 댓글 말투
* 단톡방에서 친구들이 놀리는 말투
* 릴스/틱톡에서 반복될 법한 짧은 문장
* 듣자마자 따라 하고 싶은 밈송 느낌

을 반영해라.

쓸 수 있는 밈 화법 (곡당 1~2번만):
...
* 억지 밈
* 오래된 유행어 ("헐", "대박", "킹왕짱") — 오히려 안 웃기다
* 어색한 인터넷 말투
* 맥락 없는 밈 표현

중요한 것은 유행어를 많이 쓰는 게 아니라,
짧고 찰지고 킹받는 표현을 만드는 것이다.
...
Hook은 키워드를 그대로 반복하지 말고,
키워드에서 나온 가장 웃긴 표현(별명, 한마디)을
2줄짜리 밈 문장으로 압축해라.
...
모든 키워드를 억지로 다 쓰려고 하지 마라.

가장 웃기고 킹받게 만들 수 있는 키워드 1~2개를 중심으로 잡고,
나머지 키워드는 자연스럽게 보조로 섞어라.

키워드를 많이 넣는 것보다,
하나의 캐릭터가 선명하게 보이는 것이 더 중요하다.
...
* short complete 45-second one-verse song
* no intro
* no outro
* no bridge
* repeated strong hook
* clear ending feel
* 선택된 장르의 리듬
* 선택된 장르의 악기
* 선택된 장르의 창법
...
* Hook은 반드시 A/B/A/B 또는 A/B/A/B' 반복 구조로 작성해라
...
"style": "선택된 장르의 특징이 반영된 영어 Suno AI 스타일 설명. short complete 45-second one-verse song, no intro, no outro, no bridge, repeated strong hook, clear ending feel, genre-specific rhythm, instruments, vocal style, melody feel 포함",
"lyrics": "[Verse]\n...\n...\n...\n...\n\n[Pre-Hook]\n...\n...\n\n[Hook]\nA\nB\nA\nB'"
}
`;
}
